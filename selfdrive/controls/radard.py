...
class RadarD:
  ...
  def get_lead(self, md, tracks: dict[int, Track], index: int, lead_msg: capnp._DynamicStructReader,
    ...
    # 获取纵向控制模式配置
    from openpilot.common.params import Params
    longitudinal_mode = Params().get_int("LongitudinalControlMode")  # 0:视觉, 1:雷达, 2:融合
    ...
    elif longitudinal_mode == 2:  # 雷达视觉融合模式
        if track is not None and byd_radar_track is not None and ready and lead_msg.prob > .5:
            # 融合策略
            vision_lead = self.vision_tracks[index].get_lead(md)
            radar_lead = byd_radar_track.get_RadarState(md, lead_msg.prob, self.vision_tracks[0].yRel)

            if abs(radar_lead['dRel'] - vision_lead['dRel']) < 10.0:
                lead_dict = {
                    'dRel': radar_lead['dRel'] * 0.7 + vision_lead['dRel'] * 0.3,
                    'vRel': vision_lead['vRel'],
                    'aLead': vision_lead['aLead'],
                    'status': True,
                    'radar': True,
                    'modelProb': lead_msg.prob
                }
            else:
                lead_dict = vision_lead  # 距离差异太大时使用视觉
        elif track is not None:
            lead_dict = track.get_RadarState(md, lead_msg.prob, self.vision_tracks[0].yRel)
